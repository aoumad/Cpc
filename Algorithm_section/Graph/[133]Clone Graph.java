...
// 
//
// Related Topics Hash Table Depth-First Search Breadth-First Search Graph 👍 69
//70 👎 2877
...
class Solution {
    public Node cloneGraph(Node node)
    {
        ...
        queue.add(node);
        deepCopy.put(node, new Node(node.val));
        while(!queue.isEmpty())
        {
            Node temp = queue.poll();
            if (temp.neighbors.size() == 0 && !deepCopy.containsKey(temp))
            {
                deepCopy.put(temp, new Node(temp.val));
                queue.add(temp);
            }
            else
            {
                for(Node curr : temp.neighbors)
                {
                    ...
                    {
                        deepCopy.get(temp).neighbors.add(deepCopy.get(curr));
                    }
                }
            }
        }
